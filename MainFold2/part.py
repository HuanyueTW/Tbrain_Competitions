...
for i in os.listdir("json"):
    with open("json/" + i, encoding="utf-8") as f:
        ...
        for j in range(0, len(data['shapes'])): 
            if (data['shapes'][j]['group_id'] in [1, 4]): #中文字元
                ...
                # if (data['shapes'][j]['points'][0][0] > data['shapes'][j]['points'][2][0]):
                #     img_after = img_ori[ data['shapes'][j]['points'][0][1] : data['shapes'][j]['points'][2][1], data['shapes'][j]['points'][2][0] : data['shapes'][j]['points'][0][0]]
                #     a = a + "e"               
                # else:
